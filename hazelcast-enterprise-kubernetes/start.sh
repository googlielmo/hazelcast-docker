...
then
	...
	java -server $JAVA_OPTS -Dhazelcast.shutdownhook.policy=GRACEFUL -Dhazelcast.config=$FILE -Dhazelcast.http.healthcheck.enabled=true -Djava.net.preferIPv4Stack=true -Dhazelcast.enterprise.license.key=$HZ_LICENSE_KEY com.hazelcast.core.server.StartServer
else
	echo "no custom configuration found"
	java -server $JAVA_OPTS -Dhazelcast.shutdownhook.policy=GRACEFUL -Dhazelcast.http.healthcheck.enabled=true -Djava.net.preferIPv4Stack=true -Dhazelcast.enterprise.license.key=$HZ_LICENSE_KEY com.hazelcast.core.server.StartServer
fi
